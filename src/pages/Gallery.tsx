...
export const Gallery: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'masonry' | 'polaroid' | 'grid' | 'slideshow'>('masonry');
  ...
  const fetchGallery = () => {
    fetchWithFallback<GalleryItem[]>('/api/gallery', STORAGE_KEY_GALLERY, defaultGalleryItems, 'gallery')
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let mediaType: 'photo' | 'video' = editingItem ? editingItem.type : 'photo';
      ...
      if (editingItem) {
        const updatedItem = { ...editingItem, type: mediaType, url: finalUrl, caption, date };
        ...
        if (isSupabaseConfigured() && client) {
          try {
            ...
            if (error) console.error('Supabase update gallery error:', error.message, error);
          } catch (err) {
        ...
        if (isSupabaseConfigured() && client) {
          try {
            ...
            if (error) {
              console.error('Supabase insert gallery error:', error.message, error);
            } else if (data && data[0]) {
              const inserted = data[0] as unknown as GalleryItem;
              const synced = [inserted, ...items.filter((i) => i.id !== newItem.id)];
              saveItems(synced);
            }
          } catch (err) {
  ...
  const toggleFavorite = async (id: number) => {
    ...
    const updated = items.map((it) => (it.id === id ? { ...it, favorite: newFav } : it));
    saveItems(updated);

    const client = getSupabaseClient();
    if (isSupabaseConfigured() && client) {
      try {
        const { error } = await client.from('gallery').update({ favorite: newFav }).eq('id', id);
        if (error) console.error('Supabase favorite gallery error:', error.message, error);
      } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (isSupabaseConfigured() && client) {
      try {
        ...
        if (error) console.error('Supabase delete gallery error:', error.message, error);
      } catch (err) {
  ...
  const filteredItems = items.filter((item) => (showOnlyFav ? item.favorite === 1 : true));
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-[#5C3A4D]/80 text-sm max-w-lg mx-auto">
          Captured smiles, quiet sunsets, and beautiful moments preserved forever in time.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white/60 backdrop-blur-md p-3 sm:p-4 rounded-3xl border border-white/80 shadow-sm">
        {/* View Mode Selectors */}
        <div className="flex items-center gap-1 bg-[#FFE4EC]/60 p-1 rounded-full overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden max-w-full">
          {(['masonry', 'polaroid', 'grid', 'slideshow'] as const).map((mode) => (
            <button
              key={mode}
              onClick={() => setViewMode(mode)}
              className={`px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-wider capitalize shrink-0 transition whitespace-nowrap ${
                viewMode === mode
        ...
        <div className="flex items-center justify-end gap-2 shrink-0 pt-1 sm:pt-0 border-t sm:border-t-0 border-pink-100/60">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition border shrink-0 ${
              showOnlyFav
          ...
          >
            ...
            <span className="sm:hidden text-[11px]">Fav</span>
          </button>

          <button
            onClick={openUploadModal}
            className="flex items-center gap-1.5 px-3.5 sm:px-5 py-2 rounded-full bg-[#EC407A] hover:bg-[#D81B60] text-white text-xs font-bold uppercase tracking-widest shadow-md transition transform active:scale-95 shrink-0"
          >
            <Upload size={14} />
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-16 px-6 bg-white/60 backdrop-blur-md rounded-3xl border border-white/80 shadow-sm space-y-4 max-w-md mx-auto">
          ...
          <h3 className="font-serif text-lg font-bold text-[#5C3A4D]">Belum Ada Foto/Video</h3>
          <p className="text-xs text-[#5C3A4D]/80">
            Galeri kamu masih kosong. Tambahkan momen foto atau video indah kalian sekarang!
          </p>
          <button
            onClick={openUploadModal}
          ...
          {filteredItems.length > 0 ? (
            <div className="relative w-full h-[380px] sm:h-[450px] flex items-center justify-center">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-black/60 backdrop-blur-md p-3 rounded-xl text-white text-center">
                <p className="text-sm font-medium">{filteredItems[slideshowIndex].caption}</p>
                <p className="text-xs text-pink-300">{filteredItems[slideshowIndex].date}</p>
              </div>
              ...
              <button
                onClick={() =>
                  setSlideshowIndex(
                    (slideshowIndex - 1 + filteredItems.length) % filteredItems.length
                  )
                }
            ...
            <div className="text-pink-300 text-sm">No items to display in slideshow</div>
          )}
        </div>
      ) : viewMode === 'polaroid' ? (
        /* Polaroid Mode */
            ...
            >
              ...
              <div className="flex items-center justify-between text-pink-950 font-serif">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    <Heart size={16} className={item.favorite ? 'fill-rose-500' : ''} />
                  </button>
                  <button
                ...
                <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/20">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => openEditModal(item)} title="Edit">
                      <Edit2 size={16} className="text-white hover:text-pink-300" />
                    </button>
                    <button onClick={() => toggleFavorite(item.id)}>
                      <Heart
                        size={16}
                        className={item.favorite ? 'fill-rose-500 text-rose-500' : 'text-white'}
                      />
                    </button>
                    <button onClick={() => handleDelete(item.id)}>
                      <Trash2 size={16} className="text-rose-300 hover:text-rose-500" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleFileUpload} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-pink-900 mb-1">
                    Select File {editingItem ? '(Optional - Leave blank to keep current media)' : '(JPG, PNG, WEBP, MP4, WEBM)'}
                  </label>
                  <input
                    type="file"
                    accept="image/*,video/*"
        ...
        >
          ...
          >
            ...
            <div className="text-center mt-4 text-white">
              ...
              <p className="text-xs text-pink-300">{filteredItems[lightboxIndex].date}</p>
            </div>
          </div>
        </div>
      )}
